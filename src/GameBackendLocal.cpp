...
public:
	...
	void OnPacket(std::shared_ptr<NodeStatePacket> p) {
		...
		backend->enqueueState(p->netid, p->x, p->y, p->z, p->yaw, p->team);
...
void GameBackendLocal::broadcastState(uint32_t netid, float x, float y, float z, float yaw, uint8_t team) {
