...
GameBackendServer::GameBackendServer(const std::string& bindIp, uint16_t port, const std::string& serverName, int teamSize, const std::string& masterIp, uint16_t masterPort, const std::string& publicIp, bool useP2P)
...
void GameBackendServer::start() {
    ...
    registerWithMasterServer(serverName, false, "", targetMasterIp, targetMasterPort, publicIp, useP2P);
}
