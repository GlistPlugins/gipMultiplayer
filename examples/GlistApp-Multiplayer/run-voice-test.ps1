...
$buildDir = Join-Path $env:TEMP "gipmultiplayer-voice-example-$($Configuration.ToLowerInvariant())"
...
if ($LASTEXITCODE -ne 0) {
	throw "Voice example configuration failed"
}

& $cmake --build $buildDir --config $Configuration
if ($LASTEXITCODE -ne 0) {
	throw "Voice example build failed"
}
