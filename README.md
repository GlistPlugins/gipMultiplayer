...
In `examples/GlistApp-Multiplayer`, select `[3]` from the startup menu. On the audio screen, `[1]` starts delayed raw PCM loopback, `[2]` starts the Opus encode/decode loopback, `Space` stops or restarts it, and `Esc` returns to the menu.

On Windows, configure, build and launch the microphone test from the repository root with:

```powershell
& .\examples\GlistApp-Multiplayer\run-voice-test.ps1
```
